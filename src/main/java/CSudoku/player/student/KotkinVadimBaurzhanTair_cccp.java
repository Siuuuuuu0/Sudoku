...
// import CSudoku.board.Constraint;
...
public class KotkinVadimBaurzhanTair_cccp implements MoveStrategy {
    ...
    private final ConcurrentHashMap<Long, TranspositionTableEntry> transpositionTable;
    ...
    private static class TranspositionTableEntry {
         ...
         */
        TranspositionTableEntry(int score, int depth, Flag flag) { // Added flag
            this.score = score;
            this.depth = depth;
            this.flag = flag;
        }
     ...
     */
    public KotkinVadimBaurzhanTair_cccp() {
    ...
    private int minimax(EvaluatedSimulatedBoard board, int depth, int alpha, int beta, boolean isMaximizingPlayer , Player player) {
        ...
        if (!timeUp) {
            ...
            if (bestValue <= originalAlpha)       flag = TranspositionTableEntry.Flag.UPPER_BOUND; // Failed low (didn't raise alpha)
            else if (bestValue >= beta)           flag = TranspositionTableEntry.Flag.LOWER_BOUND; // Failed high (exceeded beta)
            else                                  flag = TranspositionTableEntry.Flag.EXACT;   // Score is within the [alpha, beta] window

            // Create new entry and store it using compute for atomic update
            TranspositionTableEntry newEntry = new TranspositionTableEntry(bestValue, depth, flag);
    ...
    protected void finalize() throws Throwable {
        try {
            if (!executorService.isShutdown()) {
                // System.err.println("KotkinVadimTairBaurzhan_cccp finalize: ExecutorService was not shut down. Forcing shutdown.");
                shutdownExecutor();
            }
        } finally {
            super.finalize();
     ...
     * </p>
     *
     * @return The name of the strategy, which is {@code "cccp Student"}.
     */
     ...
     */
    private static class EvaluatedSimulatedBoard extends CSudokuBoard {
        ...
        public int setValue(Move move, boolean isMaximisingPlayer) {
            ...
            // penalisation
            if (this.zerosInRows[row] == 1) delta -= bns;
            if (this.zerosInColumns[col] == 1) delta -= bns;
            if (this.isSubgridAlmostFilled(row, col)) delta -= bns;
